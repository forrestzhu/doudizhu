# Feature Specification: Core Harness And Pluggable Decisions
...
1. **Given** a fixed seed, **When** the harness runs one game, **Then** it reports the winner and turn count.
2. **Given** multiple fixed seeds, **When** the harness runs multiple games, **Then** it reports aggregate win counts.
...
- **FR-008**: System MUST provide a CLI harness for repeated seeded self-play.
...
- **SC-001**: `cargo test` verifies core rule, deal, visibility, and harness behavior.
- **SC-002**: `cargo run --bin harness -- --games 10 --seed 42` completes without UI dependencies.
